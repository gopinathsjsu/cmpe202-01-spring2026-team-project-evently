...
function formatLocation(loc: EventFromApi["location"], isOnline: boolean): string {
...
const FOOTER_LINKS = {
  ...
  "Plan Events": ["Event Planning", "Sell Tickets", "Event Marketing", "Resources"],
    ...
    <div className="min-h-screen bg-white text-black font-sans antialiased">
      ...
      <main>
        ...
        <div className="mx-auto max-w-7xl px-4 text-left sm:px-6 lg:px-8">
            <h1 className="text-3xl font-bold tracking-tight text-black sm:text-4xl">Discover Events That Matter</h1>
            <p className="mt-3 text-lg text-gray-600">Find and attend events near you or create your own</p>
            <form onSubmit={handleSearch} className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-stretch sm:justify-start">
              <div className="relative min-w-0 sm:min-w-[200px] sm:max-w-[280px] rounded-md border border-gray-300 bg-gray-50">
                ...
                <input type="search" placeholder="Search for events" className="w-full rounded-md bg-transparent py-3 pl-11 pr-4 text-base placeholder:text-gray-500 focus:border-black focus:outline-none focus:ring-0" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
              <div className="relative min-w-0 sm:min-w-[200px] sm:max-w-[280px] rounded-md border border-gray-300 bg-gray-50">
                <MapPinIcon className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400" />
                <input type="text" placeholder="Location" className="w-full rounded-md bg-transparent py-3 pl-11 pr-4 text-base placeholder:text-gray-500 focus:border-black focus:outline-none focus:ring-0" value={locationQuery} onChange={(e) => setLocationQuery(e.target.value)} />
              </div>
              <button type="submit" className="rounded-md border border-black bg-black px-6 py-3 text-base font-medium text-white hover:bg-gray-800">Search</button>
            </form>
          </div>
        </section>
            ...
            <div className="mt-6 flex flex-wrap justify-between gap-4 pb-2">
              {CATEGORIES.map((cat) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setCategoryFilter((c) => (c === cat.name ? null : cat.name));
                    setPage(1);
                  }}
                  className={`flex min-w-[120px] flex-col items-center gap-2 rounded-lg border px-6 py-4 text-center transition-colors hover:border-gray-400 hover:bg-gray-50 ${
                    categoryFilter === cat.name ? "border-black bg-gray-50" : "border-gray-300 bg-white"
                  }`}
                >
            ...
            <aside className="hidden w-56 shrink-0 lg:block">
              <div className="rounded-lg border border-gray-200 bg-white p-4">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={handleClearAll} className="text-sm text-blue-600 hover:text-blue-700">Clear all</button>
                </div>
                <div>
                  <h4 className="mt-4 font-semibold text-black">Date</h4>
                  <ul className="mt-3 space-y-2">
                    {["Today", "This Week", "This Month", "Custom Date"].map((opt) => (
                      <li key={opt} className="flex items-center gap-2">
                        <input type="checkbox" id={`date-${opt}`} className="h-4 w-4 rounded border-gray-300 text-black focus:ring-black" checked={dateFilters.includes(opt)} onChange={(e) => setDateFilters((prev) => e.target.checked ? [...prev, opt] : prev.filter((x) => x !== opt))} />
                        <label htmlFor={`date-${opt}`} className="text-sm text-gray-700">{opt}</label>
                      </li>
                    ))}
                  </ul>
                </div>
                <div className="mt-6">
                  ...
                  <ul className="mt-3 space-y-2">
                    {["In-Person", "Online"].map((opt) => (
                      <li key={opt} className="flex items-center gap-2">
                        <input type="checkbox" id={`type-${opt}`} className="h-4 w-4 rounded border-gray-300 text-black focus:ring-black" checked={eventTypeFilters.includes(opt)} onChange={(e) => setEventTypeFilters((prev) => e.target.checked ? [...prev, opt] : prev.filter((x) => x !== opt))} />
                        <label htmlFor={`type-${opt}`} className="text-sm text-gray-700">{opt}</label>
                      </li>
                    ))}
                  </ul>
                </div>
                <div className="mt-6">
                  ...
                  <ul className="mt-3 space-y-2">
                    {["Free", "Paid"].map((opt) => (
                      <li key={opt} className="flex items-center gap-2">
                        <input type="radio" name="price" id={`price-${opt}`} className="h-4 w-4 border-gray-300 text-black focus:ring-black" checked={priceFilter === opt} onChange={() => setPriceFilter(opt)} />
                        <label htmlFor={`price-${opt}`} className="text-sm text-gray-700">{opt}</label>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="min-w-0 flex-1">
              <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value as "start_time" | "price" | "title")}
                  >
              ...
              {loading ? (
                <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div key={i} className="overflow-hidden rounded-lg border border-gray-200 bg-gray-100 animate-pulse">
                ...
                <p className="py-12 text-center text-gray-500">No events found. Try different filters or search.</p>
              ) : (
                <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
                  {events.map((event) => (
                    <article key={event.id} className="overflow-hidden rounded-lg border border-gray-200 bg-white transition-shadow hover:shadow-md">
                      <div className="aspect-[16/10] w-full bg-gray-200" aria-label="Event image">
                        {event.image_url && (
                          <img src={event.image_url} alt="" className="h-full w-full object-cover" />
                        )}
                      </div>
                      <div className="relative p-4">
                        <button type="button" className="absolute right-4 top-4 text-gray-400 hover:text-red-500" aria-label="Favorite">
                          <HeartIcon filled={false} className="h-5 w-5" />
                        </button>
                        <p className="text-xs font-medium uppercase tracking-wide text-gray-500">{formatEventDate(event.start_time)}</p>
                        <h3 className="mt-1 text-lg font-bold text-black">{event.title}</h3>
                        <p className="mt-1 flex items-center gap-1 text-sm text-gray-600">
                          {event.is_online ? <GlobeIcon className="h-4 w-4 shrink-0" /> : <MapPinIcon className="h-4 w-4 shrink-0" />}
                          <span>{formatLocation(event.location, event.is_online)}</span>
                        </p>
                        <p className="mt-2 text-sm font-medium text-black">{formatPrice(event.price)}</p>
                        <p className="mt-1 text-sm text-gray-500">{formatAttending(event.attending_count)}</p>
                      </div>
                    </article>
                  ))}
                </div>
        ...
        <section className="border-b border-gray-200 bg-white py-16 text-center">
          <div className="mx-auto max-w-2xl px-4">
            <h2 className="text-3xl font-bold text-black">Host Your Own Event</h2>
            <p className="mt-3 text-lg text-gray-600">Create and manage events with our easy-to-use platform.</p>
            <a href="/create" className="mt-6 inline-block rounded-md bg-black px-6 py-3 text-base font-medium text-white hover:bg-gray-800">
              Create Event
            </a>
          </div>
        </section>
              ...
              <div>
                ...
                <div className="mt-4 flex gap-4">
                  {["Facebook", "Twitter", "Instagram", "LinkedIn"].map((name) => (
                    <a key={name} href="#" className="text-sm hover:text-black" aria-label={name}>{name}</a>
                  ))}
                </div>
              </div>
            </div>
