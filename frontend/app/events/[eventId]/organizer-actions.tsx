"use client";

import Link from "next/link";

import { useAuth } from "@/lib/auth";

interface OrganizerActionsProps {
  eventId: number;
  organizerUserId: number;
  attendingCount: number;
  totalCapacity: number;
}

function UsersIcon({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
      strokeWidth={2}
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"
      />
    </svg>
  );
}

export function OrganizerActions({
  eventId,
  organizerUserId,
  attendingCount,
  totalCapacity,
}: OrganizerActionsProps) {
  const { user, loading } = useAuth();

  if (loading) return null;
  if (!user) return null;

  const isOrganizer = user.id === organizerUserId || user.roles.includes("admin");
  if (!isOrganizer) return null;

  return (
    <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-4">
      <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-indigo-600">
        Organizer Tools
      </p>
      <div className="mb-3 flex items-center justify-between text-sm text-indigo-800">
        <span>Attendees</span>
        <span className="font-bold">
          {attendingCount} / {totalCapacity}
        </span>
      </div>
      <Link
        href={`/events/${eventId}/attendees`}
        className="flex w-full items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700"
      >
        <UsersIcon className="h-4 w-4" />
        Manage Attendees
      </Link>
    </div>
  );
}
